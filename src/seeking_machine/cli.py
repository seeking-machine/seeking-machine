...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Seeking-Machine thinks for you about the meanings.",
        epilog="Example:  seeking-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | seeking-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | seeking-machine         #

    $ seeking-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="seeking-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"seeking-machine: {utterance}.")
        syslog.closelog()
